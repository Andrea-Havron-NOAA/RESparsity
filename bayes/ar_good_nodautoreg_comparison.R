...
nll <- function(par) {
  ...
  #                  scale = exp(logSdR),
  #                  log   = TRUE)
  ## Explicit AR(1) conditionals matching dautoreg convention exactly:
  ##   x[1]   ~ N(mu, scale)
  ##   x[i]   ~ N(phi*x[i-1] + mu*(1-phi), scale*sqrt(1-phi^2))
  ## Same density as dautoreg but written as a loop -- dense Hessian.
  phiAR <- 2 * plogis(tphiR) - 1
  sdAR  <- exp(logSdR)
  ans <- -dnorm(logN1Y[1], logMuR, sdAR, log = TRUE)
  ans <- ans - dnorm(logN1A[1],
                     phiAR * logN1Y[1] + logMuR * (1 - phiAR),
                     sdAR * sqrt(1 - phiAR^2), log = TRUE)
  for (i in 2:(ny - 1))
    ans <- ans - dnorm(logN1A[i],
                       phiAR * logN1A[i - 1] + logMuR * (1 - phiAR),
                       sdAR * sqrt(1 - phiAR^2), log = TRUE)
...
saveRDS(results, "ar_good_nodautoreg_results.rds")
