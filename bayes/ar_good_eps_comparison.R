## ============================================================
## AR good (eps): eps ~ N(0, sdAR*sqrt(1-phi^2)) centred, length ny-1
## ============================================================
...
nll <- function(par) {
  ...
  ## AR(1) with centred deviations -- parallel to MA good
  ## eps ~ N(0, sdAR*sqrt(1-phi^2)), matching dautoreg conditional sd
...
transformed parameters {
  ...
  matrix[na, ny] logN;
  vector[na] logFA;
  vector[ny - 1] logN1A;     // deterministic from AR recursion
  ...
  // AR(1) recursion: logN1A deterministic given eps
  logN1A[1] = phi * logN1Y[1] + logMuR * (1 - phi) + eps[1];
  for (i in 2:(ny - 1))
    logN1A[i] = phi * logN1A[i - 1] + logMuR * (1 - phi) + eps[i];
...
model {
  // eps ~ N(0, sdR*sqrt(1-phi^2)), logN1Y[1] ~ N(logMuR, sdR)
  // matching dautoreg convention
  logN1Y[1] ~ normal(logMuR, sdR);
  eps ~ normal(0, sdR * sqrt(1 - phi^2));
...
for (i in 1:n_sim_iter) {
  ...
  get_inits <- function(chain_id) {
    ...
    # subset logFA to only the 4 estimated
