...
## Parameter list ############################################
## eps -> eps_std: the non-centred innovation, N(0,1) prior
## eps = eps_std * exp(logSdR)
par <- list(
  logN1Y = rep(0, nrow(dat$M)),
  eps_std = rep(0, ncol(dat$M)),
...
nll <- function(par) {
  ...
  ## --- Non-centered innovations: eps_std ~ N(0,1), eps = eps_std * sdR
  sdR <- exp(logSdR)
  eps <- eps_std * sdR

  ## --- Priors
  ## Non-centered prior on innovations
  ans <- -sum(dnorm(eps_std, 0, 1, log = TRUE))
...
## Build TMB object and do ML estimation
...
data {
  ...
  int<lower=1> n_survey_ages;  // number of ages (= 5)
  int<lower=1> min_age;
...
parameters {
  vector[na] logN1Y;
  vector[ny] eps_std;              // non-centered: eps = eps_std * sdR
  vector[ny] logFY;
...
#     pvec[par_names == "eps_std"] <- ap$eps_std[row, ]
#     pvec
#   }
#
...
#
#   ## Joint NLL via obj$env$f() (no Laplace)
...
saveRDS(results, "bayes/ma_noncentered_results.rds")
