...
library(tictoc)
par <- list(
  logN1Y=rep(0,nrow(dat$M)), #initial naa
  x = rep(0, nrow(dat$M) * ncol(dat$M) - nrow(dat$M) - ncol(dat$M) + 1),
...
nll<-function(par){
  ...
  for(y in 2:ny){
    ...
    for(a in 2:na){
      idx <- y * na + a
      logN[a,y] <- logN[a-1,y-1]-F[a-1,y-1]-M[a-1,y-1] * x[idx]
    }
  }

  residuals <- logNAA - logN[2:na, 2:ny]
  unit_var <- exp(logSdNAA) / sqrt( 1 - plogis(tphiNAAy)^2) / sqrt(1 - plogis(tphiNAAa)^2)
  f1 <- function(x) dautoreg(x, mu = 0, phi = plogis(tphiNAAa), TRUE)
  f2 <- function(x) dautoreg(x, mu = 0, phi = plogis(tphiNAAy), TRUE)
  ans <- ans - RTMB::dseparable(f1,f2)(residuals, scale = unit_var)
  ...
  for(i in 1:length(logObs)){
    ...
    y <- year[i]-min(year)+1
    if(fleet[i]==1){
      logPred[i] <- log(F[a,y])-log(F[a,y]+M[a,y])+log(1-exp(-F[a,y]-M[a,y]))+logNAA[a,y]
      sdvec[i] <- exp(logSdCatch)
    }else{
      logPred[i] <- logQ[a]-(F[a,y]+M[a,y])*surveyTime+logNAA[a,y]
