...
times <- 0
  ...
  time_centered_no_int_min <- min(times)
  time_centered_no_int_max <- max(times)
```
...
times[i] <- as.numeric(t2$toc - t2$tic)
  ...
  time_centered_int_sd <- sd(times)
    time_centered_int_min <- min(times)
  time_centered_int_max <- max(times)
```
...
times[i] <- as.numeric(t2$toc - t2$tic)
  ...
  time_noncentered_no_int_sd <- sd(times)
    time_noncentered_no_int_min <- min(times)
  time_noncentered_no_int_max <- max(times)
```
...
times[i] <- as.numeric(t2$toc - t2$tic)
  ...
  time_noncentered_int_sd <- sd(times)
    time_noncentered_int_min <- min(times)
  time_noncentered_int_max <- max(times)
```
...
m <- data.frame("Intercept" = c("Y","N","Y","N"),
                ...
                "Min" = round(c(time_centered_no_int_min, time_centered_int_min, time_noncentered_no_int_min, time_noncentered_int_min), 3),
                "Max" = round(c(time_centered_no_int_max, time_centered_int_max, time_noncentered_no_int_max, time_noncentered_int_max), 3))
knitr::kable(m)
```
