...
par$z <-  rnorm(length(dat$year), 0, 0.1)#rep(0, length(dat$year))
##############
...
jnll<-function(par){
  ...
  for(y in 2:nrow){
    for(a in 2:ncol){
      predN <- logN_rec[y-1,a-1]-exp(logFF[y-1,a-1])-M[y-1,a-1]
      if(a==ncol){
        predN <- log(exp(predN)+exp(logN_rec[y-1,a]-exp(logFF[y-1,a])-M[y-1,a]))
      }
      jnll <- jnll - dnorm(logN_rec[y,a],predN,sdS,log=TRUE)
    }
  }
  ...
  for(i in 1:nobs){
    ...
    if(fleetTypes[f]==0){
      # Change logN
      logPred[i] <- logN_rec[y,a]-log(Z)+log(1-exp(-Z))+logFF[y,a]
    }
    if(fleetTypes[f]==2){
      # Change logN 
      logPred[i] <- logQ[keyQ[f,a]+1]+logN_rec[y,a]-Z*sampleTimes[f]
    }
  }
